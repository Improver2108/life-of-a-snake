...
const CanvasBoard = ({ height, width }: ICanvasBoard) => {
  ...
  const [foodPos, setFoodPos] = useState<IObjectBody>(
  ...
  const movingDirection = useSelector(
    (state: RootState) => state.snake.movingDirection
  );
  ...
  useEffect(() => {
    ...
    if (
      snake1.pos[0].x >= width ||
      snake1.pos[0].x <= -20 ||
      snake1.pos[0].y >= height ||
      snake1.pos[0].y <= -2 ||
      collideWithItself(snake1.pos[0], snake1.pos)
    ) {
    ...
    (event: KeyboardEvent) => {
      ...
      if (movingDirection) {
        if (key === "d") dispatch(changeDirection(["right", "left", 20, 0]));
          ...
          dispatch(changeDirection(["left", "right", -20, 0]));
